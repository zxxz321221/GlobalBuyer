...
/** 内容提示文字 */
@property (assign, nonatomic) NSString * amountTitle;

@end

@implementation PasswordViewSuper

- (instancetype)initWithAlertTitle:(NSString *)alertTitle AlertMessageTitle:(NSString *)messageTitle AlertAmount:(NSString *)alertAmount {
...
+ (instancetype)wh_passwordViewWithAlertTitle:(NSString *)alertTitle AlertMessageTitle:(NSString *)messageTitle AlertAmount:(NSString *)alertAmount {
    return [[self alloc] initWithAlertTitle:alertTitle AlertMessageTitle:messageTitle AlertAmount:alertAmount];
    ;
}
...
- (void)setupPasswordView {
    ...
    [self.coverView addTarget:self action:@selector(cancelBtnClick) forControlEvents:UIControlEventTouchUpInside];
    // 顶部View
    ...
    marginView.backgroundColor = Main_Color;
    ...
    if (self.amountTitle) {
        amount_label.text = [NSString stringWithFormat:@"%@",self.amountTitle];
    }else{
...
- (void)passwordCompleteInput:(WHPayPasswordView *)password {
    ...
    [self.delegate didEnterPWD:[NSString stringWithFormat:@"%@",password.saveStore] withType:self.setOrEnter];
