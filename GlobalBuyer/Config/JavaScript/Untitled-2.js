...
function walkParserDom(doms) {
    for (var i in doms) {
        if (doms[i].type == 'text') {
            ...
            text = escape(text).replace(/(%26nbsp%3B)/g, ' ');
            text = unescape(text);
    ...
    if (node != null) {
        while (node != null) {
            if (node.nodeType == 3) {
                ...
                node.nodeValue = escape(node.nodeValue).replace(/(%A0)/g, ' ');
...
translatorWidget.style.display = 'block';