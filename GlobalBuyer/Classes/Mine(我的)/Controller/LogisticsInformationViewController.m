...
- (void)downData
{
    ...
    NSDictionary *params = @{@"api_id":LogisticsAPI_ID,
                             ...
                             @"packageNum":self.packageNum};
    AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
    [manager GET:InquiryLogisticsApi parameters:params progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath;
{
    ...
    for (int i = 0 ; i < 3; i++) {
        if (i == 0) {
            cell.dateLb.text = [NSString stringWithFormat:@"%@:%@",NSLocalizedString(@"GlobalBuyer_My_Date", nil),self.dataSource[indexPath.row][i]];
        }
        if (i == 1) {
            cell.serviceLb.text = [NSString stringWithFormat:@"%@:%@",NSLocalizedString(@"GlobalBuyer_My_ServiceLocation", nil),self.dataSource[indexPath.row][i]];
        }
        if (i == 2) {
            cell.detailLb.text = [NSString stringWithFormat:@"%@:%@",NSLocalizedString(@"GlobalBuyer_My_Detailed", nil),self.dataSource[indexPath.row][i]];
        }
    }
