//
//  ParcelDetailsViewController.m
//  GlobalBuyer
//
//  Created by 薛铭 on 2017/9/18.
...
@interface ParcelDetailsViewController ()<UITableViewDelegate,UITableViewDataSource,UIScrollViewDelegate>
...
@implementation ParcelDetailsViewController
...
{
    if (_tableView == nil) {
        _tableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 0, kScreenW, kScreenH-bottomH) style:UITableViewStylePlain];
...
- (void)sureBtn
{
    ...
    chooseVC.packageId = self.packageId;
    chooseVC.type = @"parcelDetail";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *string = self.dataSource[indexPath.row][@"get_pay_product"][@"body"];
    ...
    cell.priceLb.text = [NSString stringWithFormat:@"x%@",jsonDict[@"quantity"]];
    cell.priceLb.textColor = [UIColor blackColor];
    
    
    NSString *inspectionPictureUrl = [NSString stringWithFormat:@"%@",self.dataSource[indexPath.row][@"confirm_picture"]];
...
{
    NSString *inspectionPictureUrl = [NSString stringWithFormat:@"%@",self.dataSource[btn.tag - 800][@"confirm_picture"]];
    NSArray *numUrl = [inspectionPictureUrl componentsSeparatedByString:@","];
    
    self.imageSV = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 0, kScreenW, kScreenH)];
    ...
    self.currentPage = [[UIPageControl alloc]initWithFrame:CGRectMake(0, kScreenH - 100 -kNavigationBarH, kScreenW, 30)];
