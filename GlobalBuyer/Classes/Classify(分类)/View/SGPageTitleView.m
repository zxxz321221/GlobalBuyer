...
/** 按钮之间的间距 */
static CGFloat const SGPageTitleViewBtnMargin = 15;
...
//        _indicatorView.backgroundColor = [Main_Color];
        NSString *path = [[NSBundle mainBundle]pathForResource:@"msgline"ofType:@"png"];
        UIImage *image = [UIImage imageWithContentsOfFile:path];
        _indicatorView.layer.contents = (id)image.CGImage;
    }
...
- (UIView *)bottomSeparator {
    if (!_bottomSeparator) {
        ...
        _bottomSeparator.backgroundColor = [UIColor colorWithRed:235.0/255.0 green:235.0/255.0 blue:235.0/255.0 alpha:1];
    }
...
- (void)isTitleGradientEffectWithProgress:(CGFloat)progress originalBtn:(UIButton *)originalBtn targetBtn:(UIButton *)targetBtn {
    if (self.titleColorStateSelected) {
        ...
        NSLog(@"%f",progress);
        if (progress == 0.5) {
            originalBtn.titleLabel.textColor = [UIColor blackColor];
            targetBtn.titleLabel.textColor = Main_Color;
        }
