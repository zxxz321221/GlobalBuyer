//
//  CloseOrderDetailViewController.m
//  GlobalBuyer
//
...
#import "CloseOrderDetailViewController.h"
#import "FirstTreatCell.h"
#import "StorageCell.h"
#import "TailToBePaidCell.h"
#import "GoodsToBeReceivedCell.h"
@interface CloseOrderDetailViewController ()<UITableViewDataSource,UITableViewDelegate,UIScrollViewDelegate>
{
...
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section{
    if (section == 0) {
        return [Unity countcoordinatesH:592]+NavBarHeight+[Unity countcoordinatesH:90];
    }
...
- (UIImageView *)statusImg{
    if (!_statusImg) {
        _statusImg = [[UIImageView alloc]initWithFrame:CGRectMake(kScreenW-[Unity countcoordinatesW:102], NavBarHeight+[Unity countcoordinatesH:15], [Unity countcoordinatesW:82], [Unity countcoordinatesH:80])];
        _statusImg.image = [UIImage imageNamed:@"交易关闭"];
    }
    return _statusImg;
}
...
- (UIView *)infoView{
    if (!_infoView) {
        _infoView = [[UIView alloc]initWithFrame:CGRectMake(0, _firstView.bottom+[Unity countcoordinatesH:10], kScreenW, [Unity countcoordinatesH:70])];
        _infoView.backgroundColor = [UIColor whiteColor];
        
        NSArray * arr =@[@"创建时间",@"关闭时间"];
        for (int i=0; i<2; i++) {
        ...
        _closeTime = [Unity lableViewAddsuperview_superView:_infoView _subViewFrame:CGRectMake(kScreenW-[Unity countcoordinatesW:160], _creationTime.bottom+[Unity countcoordinatesH:10], [Unity countcoordinatesW:150], [Unity countcoordinatesH:20]) _string:@"2019.05.13 13:00:00" _lableFont:[UIFont systemFontOfSize:15] _lableTxtColor:[Unity getColor:@"#999999"] _textAlignment:NSTextAlignmentRight];
