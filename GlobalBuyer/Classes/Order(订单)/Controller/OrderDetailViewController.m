...
@interface OrderDetailViewController ()<UITableViewDataSource,UITableViewDelegate,UIScrollViewDelegate>
{
...
//验货照片 查看物流
@property (nonatomic , strong) UIButton * inspection;
...
- (void)viewDidLoad {
    ...
    NSDictionary * dict = @{@"1":@"1",@"2":@"1",@"3":@"0",@"4":@"0"};
...
- (UIView *)bottomView{
    if (!_bottomView) {
        ...
        [_bottomView addSubview:self.inspection];
        [_bottomView addSubview:self.logistics];
    }
    return _bottomView;
}
- (UIView *)navView{
...
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section{
    if (section == 0) {
        return [Unity countcoordinatesH:772]+NavBarHeight;
    }
...
- (UILabel *)behaviorL{
    if (!_behaviorL) {
        _behaviorL = [[UILabel alloc]initWithFrame:CGRectMake([Unity countcoordinatesW:20], _statusL.bottom+[Unity countcoordinatesH:20], [Unity countcoordinatesW:100], [Unity countcoordinatesH:20])];
        _behaviorL.text = @"采购中";
...
- (UIImageView *)statusImg{
    if (!_statusImg) {
        ...
        _statusImg.image = [UIImage imageNamed:@"待入仓"];
    }
    return _statusImg;
}
...
- (UIView *)infoView{
    if (!_infoView) {
        _infoView = [[UIView alloc]initWithFrame:CGRectMake(0, _firstView.bottom+[Unity countcoordinatesH:10], kScreenW, [Unity countcoordinatesH:160])];
        _infoView.backgroundColor = [UIColor whiteColor];
        
        NSArray * arr =@[@"首款付款方式",@"创建时间",@"首款付款时间",@"首款支付单号",@"首款交易单号"];
        for (int i=0; i<5; i++) {
...
- (UIButton *)inspection{
    if (!_inspection) {
        _inspection = [[UIButton alloc]initWithFrame:CGRectMake(kScreenW-[Unity countcoordinatesW:160], [Unity countcoordinatesH:10], [Unity countcoordinatesW:70], [Unity countcoordinatesH:30])];
        [_inspection addTarget:self action:@selector(checkClick) forControlEvents:UIControlEventTouchUpInside];
        [_inspection setTitle:@"验货照片" forState:UIControlStateNormal];
        _inspection.layer.borderColor = [[Unity getColor:@"#999999"] CGColor];
        _inspection.layer.borderWidth = 1.0f;
        [_inspection setTitleColor:[Unity getColor:@"#e0e0e0"] forState:UIControlStateNormal];
        _inspection.titleLabel.font = [UIFont systemFontOfSize:14];
        _inspection.layer.cornerRadius = [Unity countcoordinatesH:15];
        _inspection.layer.masksToBounds = YES;
        _inspection.userInteractionEnabled = NO;
    }
    return _inspection;
}
- (UIButton *)logistics{
    if (!_logistics) {
        _logistics = [[UIButton alloc]initWithFrame:CGRectMake(_inspection.right+[Unity countcoordinatesW:10], [Unity countcoordinatesH:10], [Unity countcoordinatesW:70], [Unity countcoordinatesH:30])];
        ...
        _logistics.backgroundColor = [Unity getColor:@"#e0e0e0"];
        ...
        _logistics.userInteractionEnabled = NO;
