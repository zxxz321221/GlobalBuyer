//
//  CommissionDefaultViewController.m
//  GlobalBuyer
//
//  Created by 薛铭 on 2017/8/10.
...
@interface CommissionDefaultViewController ()<UITextFieldDelegate>
{
...
@implementation CommissionDefaultViewController
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    UIButton * _rightButton = [[UIButton alloc] initWithFrame:CGRectMake(kScreenW - 45,100, 44, 44)];
    [_rightButton setTitle:NSLocalizedString(@"GlobalBuyer_Address_Save", nil) forState:UIControlStateNormal];
    [_rightButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [_rightButton addTarget:self action:@selector(saveBtn) forControlEvents:UIControlEventTouchUpInside];
...
- (void)saveBtn{
    ...
    parameters[@"product_num"] = self.product_num;
    parameters[@"num"] = self.numberLb.text;
    
    [manager POST:@"hhttp://buy.dayanghang.net/api/platform/web/cart/product/webapi/num" parameters:parameters progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSLog(@"保存成功  %@",responseObject);
...
- (void)downLoadCountry
{
    ...
    for (int i = 0; i < self.countryArr.count; i ++) {
        BOOL result = [self.nationalityStr caseInsensitiveCompare:self.countryArr[i][@"sign"]] == NSOrderedSame;
        if (result) {
    ...
    [manager POST:WarehouseApi parameters:params progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if ([responseObject[@"code"]isEqualToString:@"success"]) {
            ...
            for (int i = 0; i < self.warehouseArr.count ; i++) {
                BOOL result = [self.nationalityStr caseInsensitiveCompare:self.warehouseArr[i][@"country"]] == NSOrderedSame;
                if (result) {
                    ...
                    if ([self.warehouseArr[i][@"tax_radio"]isEqual: @1]) {
                        NSString * str;
                        //用if的原因（首次登陆请求的时候消费税label还没创建 所以先使用默认的计算）
                        if (self.numberLb.text==nil) {
                            str = self.numberOfGoods;
                        }else{
                            str = self.numberLb.text;
                        }
                        self.exciseTax = [NSString stringWithFormat:@"%.2f",[self.priceOfGoods floatValue]*[str floatValue]*[self.warehouseArr[i][@"tax_rate"] floatValue]];
...
{
    NSLog(@"api_id=%@ /r api_token=%@ /r fromCountry=%@/r fromStorageId=%@/r toCountry=%@/r categoryId=%@/r weight=%@/r currencyTo=%@",API_ID,TOKEN,self.consignmentPlaceId,self.loadingPlaceId,self.receivingPlaceId,@(self.categoryInt),self.kgNumLb.text,[[NSUserDefaults standardUserDefaults]objectForKey:@"currency"]);
    NSDictionary *params = @{@"api_id":API_ID,
                             @"api_token":TOKEN,
                             @"fromCountry":self.consignmentPlaceId,
...
- (UIScrollView *)backSv
{
    if (_backSv == nil) {
        ...
        _backSv.contentSize = CGSizeMake(0, 1170);
...
//            numberLb.text = [NSString stringWithFormat:@"x%@",self.tureNumberOfGoods];
//        }else{
...
//        [_titleBackV addSubview:numberLb];
        ...
        if ([self.priceOfGoods floatValue] <= 0.00){
            ...
            priceLb.text = [CurrencyCalculation getcurrencyCalculation:[self.priceOfGoods floatValue] currentCommodityCurrency:self.moneyTypeOfGoods numberOfGoods:1.0];
//            [self.numberOfGoods floatValue]
...
- (void)reloadPage{
    //商品价格右侧price
    NSLog(@"priceofgoods  == %@",self.priceOfGoods);
    if ([self.priceOfGoods floatValue] <= 0.00) {
...
- (UIView *)firstPayTotalV
{
    if (_firstPayTotalV == nil) {
        _firstPayTotalV = [[UIView alloc]initWithFrame:CGRectMake(0, self.payDetailsV.frame.origin.y + self.payDetailsV.frame.size.height + 1, [[UIScreen mainScreen] bounds].size.width, 55)];
...
{
    if (_closeBackV == nil) {
        _closeBackV = [[UIView alloc]initWithFrame:CGRectMake( 10, self.noticeV.frame.origin.y + self.noticeV.frame.size.height + 20, [[UIScreen mainScreen] bounds].size.width/2 - 20, 50)];
        ...
        //关闭
        UILabel *sureLb = [[UILabel alloc]initWithFrame:CGRectMake( 50, 10, _closeBackV.frame.size.width - 100, 30)];
...
- (void)sureClick:(UITapGestureRecognizer *)tap
{
    ...
    NSDictionary *dict = [NSDictionary dictionaryWithObjectsAndKeys:self.bodyStr,@"bodyStr",self.loadingPlaceId,@"warehouseId", nil];
