//
//  HomeSearchWithKeyViewController.m
//  GlobalBuyer
//
//  Created by 薛铭 on 2018/10/13.
...
@interface HomeSearchWithKeyViewController ()<UITableViewDelegate,UITableViewDataSource,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,UITextFieldDelegate>
...
@implementation HomeSearchWithKeyViewController
...
- (void)viewDidLoad {
    ...
    [[BaiduMobStat defaultStat] logEvent:@"event1" eventLabel:@"[搜索商品]" attributes:@{@"收货区域":[NSString stringWithFormat:@"[%@]",[[NSUserDefaults standardUserDefaults]objectForKey:@"currencyName"]]}];
    [FIRAnalytics logEventWithName:@"搜索商品" parameters:nil];
...
//        parameters[@"keyword"] = [NSString stringWithFormat:@"%@",self.keyWords];
        ...
        language = @"zh_TW";
//        parameters[@"keyword"] = [NSString stringWithFormat:@"%@",self.twKeyWords];
    }else if([currentLanguage isEqualToString:@"en"]){
        language = @"en";
//        parameters[@"keyword"] = [NSString stringWithFormat:@"%@",self.enKeyWords];
    }else if([currentLanguage isEqualToString:@"Japanese"]){
        language = @"ja";
//        parameters[@"keyword"] = [NSString stringWithFormat:@"%@",self.jpKeyWords];
    }else{
        language = @"zh_CN";
//        parameters[@"keyword"] = [NSString stringWithFormat:@"%@",self.keyWords];
    }
...
- (void)firstData
{
    ...
    AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
    // 当前时间的时间戳
    NSString *nowStr = [self getNowTimeTimestamp3];
    NSDictionary *params = @{@"appid":BaiDuAppid,
                             ...
                             @"to":@"en",
                             @"sign":[self md5:[NSString stringWithFormat:@"%@%@%@%@",BaiDuAppid,self.keyWords,nowStr,BaiDuKey]]};
    ...
    [manager POST:BaiDuTranslateApi parameters:params progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        NSLog(@"enkey = %@",self.enKeyWords);
        __block HomeSearchWithKeyViewController *weakSelf = self;
...
//        NSLog(@"keywo = %@",self.keyWords);
        __block HomeSearchWithKeyViewController *weakSelf = self;
        
        dispatch_time_t delayTime = dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1/*延迟执行时间*/ * NSEC_PER_SEC));
        
        dispatch_after(delayTime, dispatch_get_main_queue(), ^{
            [weakSelf changeStrJp];
        });
//        [self changeStrJp];
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
        self.animationBackV.hidden = YES;
...
- (UIView *)changeWebIconV
{
    if (_changeWebIconV == nil) {
        ...
        _changeWebIconV = [[UIView alloc]initWithFrame:CGRectMake(10,  10 + 35, 90, 35*5)];
        ...
        NSArray *arr = @[@"WebIconALL",@"京东分类",@"Ebay"];
        NSArray *arrName = @[@"ALL",@"京东",@"ebay"];
...
- (void)downLoadWithSearchKeyWord:(NSString *)keyWord AmazonKeyWord:(NSString *)AmazonKeyWord
{
    ...
    parameters[@"api_id"] = API_ID;
    parameters[@"api_token"] = TOKEN;
    parameters[@"keyword"] = [NSString stringWithFormat:@"%@/%@/%@",self.keyWords,AmazonKeyWord,AmazonKeyWord];
...
//                    return ;
            ...
            for (int i = 0; i < self.allDataSource.count; i++) {
                ...
                if ([self.allDataSource[i][@"good_site"]isEqualToString:@"ebay"]) {
                    [self ebayData:self.allDataSource[i]];
                }
            }
            ...
            //
            //            for (int i = 0; i < [responseObject[@"data"][@"jd"] count]; i++) {
            //                [self.jdDataSource addObject:responseObject[@"data"][@"jd"][i]];
            //            }
            //            for (int i = 0; i < [responseObject[@"data"][@"tb"] count]; i++) {
            //                [self.tbDataSource addObject:responseObject[@"data"][@"tb"][i]];
            //            }
            //
            //            for (int i = 0; i < [responseObject[@"data"][@"tmall"] count]; i++) {
            //                [self.tmDataSource addObject:responseObject[@"data"][@"tmall"][i]ni];
            //            }
            //
            //            for (int i = 0; i < [responseObject[@"data"][@"amazon-us"] count]; i++) {
            //                [self.amDataSource addObject:responseObject[@"data"][@"amazon-us"][i]];
            //            }
            //
            //
            //            if (self.jdDataSource.count == 0) {
            //                [self.searchSource removeObject:@"JD"];
            //            }
            //
            //            if (self.tbDataSource.count == 0) {
            //                [self.searchSource removeObject:@"Taobao"];
            //            }
            //
            //            if (self.tmDataSource.count == 0) {
            //                [self.searchSource removeObject:@"Tmall"];
            //            }
            //
            //            if (self.amDataSource.count == 0) {
...
//                    return ;
            ...
            //
            //            for (int i = 0; i < [responseObject[@"data"][@"tmall"] count]; i++) {
            //                [self.tmDataSource addObject:responseObject[@"data"][@"tmall"][i]];
            //            }
            //
            //            for (int i = 0; i < [responseObject[@"data"][@"amazon-us"] count]; i++) {
            //                [self.amDataSource addObject:responseObject[@"data"][@"amazon-us"][i]];
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    ...
    if (self.searching == NO) {
        ...
        //        NSInteger number = 0;
        //        for (int i = 0; i < self.searchSource.count; i++) {
        //            if ([self.searchSource[i] isEqualToString:@"JD"]) {
        //                number += self.jdDataSource.count;
        //            }
        //            if ([self.searchSource[i] isEqualToString:@"Taobao"]) {
        //                number += self.tbDataSource.count;
        //            }
        //            if ([self.searchSource[i] isEqualToString:@"Tmall"]) {
        //                number += self.tmDataSource.count;
        //            }
        //            if ([self.searchSource[i] isEqualToString:@"Amazon"]) {
        //                number += self.amDataSource.count;
        //            }
        //        }
        //
        //
        //        self.sourceNum = number;
        //        return number;
        if ([self.sourceNum isEqualToString:@"0"]) {
            ...
            return self.mxDataSource.count;
...
//            cell.price.font = [UIFont systemFontOfSize:12];
//            cell.price.textColor = [UIColor colorWithRed:255.0/255.0 green:95.0/255.0 blue:62.0/255.0 alpha:1];
//            [cell.iv sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",self.mxDataSource[indexPath.row][@"good_pic"]]] placeholderImage:[UIImage imageNamed:@"goods"]];
//            cell.price.text = [CurrencyCalculation getcurrencyCalculation:[self.mxDataSource[indexPath.row][@"good_price"] floatValue] moneytypeArr:self.moneytypeArr currentCommodityCurrency:self.mxDataSource[indexPath.row][@"good_currency"] numberOfGoods:1.0];
//            //        cell.price.text = self.allDataSource[indexPath.row][@"good_price"];
//            cell.source.image = [UIImage imageNamed:@"梅西百货"];
//            cell.nameasd.text = self.mxDataSource[indexPath.row][@"good_name"];
//            cell.goodsLink = self.mxDataSource[indexPath.row][@"good_link"];
//
//        }else if ([self.sourceNum isEqualToString:@"8"]){
//            cell.price.font = [UIFont systemFontOfSize:12];
//            cell.price.textColor = [UIColor colorWithRed:255.0/255.0 green:95.0/255.0 blue:62.0/255.0 alpha:1];
...
//            NSLog(@"+++++++%@",self.ebDataSource[indexPath.row][@"good_link"]);
//            shopDetailVC.hidesBottomBarWhenPushed = YES;
//            [self.navigationController pushViewController:shopDetailVC animated:YES];
        }
        ...
        if ([self.sourceNum isEqualToString:@"7"]) {
            ShopDetailViewController *shopDetailVC = [ShopDetailViewController new];
            shopDetailVC.link = self.mxDataSource[indexPath.row][@"good_link"];
            shopDetailVC.hidesBottomBarWhenPushed = YES;
            [self.navigationController pushViewController:shopDetailVC animated:YES];
        }
