...
-(void)updateTag:(NSInteger)index{
    ...
    _lineImg.frame=CGRectMake(index*_tabWidth, _tabHeight-tagLineheight-1, _tabWidth, tagLineheight);
}
/**
 更新标记线位置
...
-(void)updateTagLine:(NSInteger)index{
    ...
    if(_direction==horizontal){
        ...
        [UIView animateWithDuration:0.1 animations:^{
            _lineImg.frame=CGRectMake(index*_tabWidth, _tabHeight-tagLineheight-0.5, _tabWidth, tagLineheight);
        ...
        //标记线切换动画
        [UIView animateWithDuration:0.1 animations:^{
            _lineImg.frame=CGRectMake(_tabWidth-tagLineheight-0.5, _tabHeight*index, tagLineheight, _tabHeight);
        } completion:^(BOOL finished) {
            _tagIndex=index;
...
-(void)initView{
    ...
    self.lineImg.image = [UIImage imageNamed:@"msgline"];
